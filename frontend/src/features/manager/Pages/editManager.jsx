...
const EditManager = () => {
  ...
  const [manager,setManager]=useState({picture:'',name:'',phone:'',email:'',password:'',status:'',officeId:'',departmentId:0});
  ...
  useEffect(()=>{
    const fetchManager=async ()=>{
        console.log('hit...')
        try {
            const response=await axios.get(adminApi+`/get-manager/${userId}`,{
                headers: {
      ...
      <Box component="div">
        ...
        <Formik
            ...
            console.log(values)
              ...
              const response=await axios.put(adminApi+`/edit-manager/${userId}`,formdata,{
                headers: {
                  authorization: `Bearer ${token}`,
                },
              })
          ...
          <form onSubmit={handleSubmit}>
            <FormControl>
              <Box component="div" className={classes.employee}>
                <AddManagerLeft manager={manager} touched={touched} offices={offices} handleSubmit={handleSubmit} handleBlur={handleBlur} handleChange={handleChange} errors={errors} values={values}/>
